...
export default class DomaineList extends Component {
  ...
  findAllDomaines(currentPage) {
    ...
    axios
      .get(
        "http://localhost:8081/api/domaines?pageNumber=" +
          currentPage +
          "&pageSize=" +
          this.state.domainesPerPage +
          "&sortBy=id&sortDir=" +
          sortDir
      )
  ...
  deleteDomaine = (domaineId) => {
    axios
      .delete("http://localhost:8081/api/domaines/" + domaineId)
      .then((response) => {
        if (response.data != null) {
          this.setState({ show: true });
          setTimeout(() => this.setState({ show: false }), 3000);
          this.setState({
            domaines: this.state.domaines.filter(
              (domaine) => domaine.id !== domaineId
            ),
          });
        } else {
          this.setState({ show: false });
  ...
  lastPage = () => {
    let condition = Math.ceil(
      this.state.totalElements / this.state.domainesPerPage
    );
  ...
  nextPage = () => {
    if (
      ...
      Math.ceil(this.state.totalElements / this.state.domainesPerPage)
    ) {
      if (this.state.search) {
  ...
  searchData = (currentPage) => {
    ...
    axios
      .get(
        "http://localhost:8081/api/domaines/search/" +
          this.state.search +
          "?page=" +
          currentPage +
          "&size=" +
                      ...
                      <td>
                        <ButtonGroup>
                          ...
                          <Button
                            size="sm"
                            variant="outline-danger"
                            onClick={this.deleteDomaine.bind(this, domaine.id)}
                          >
